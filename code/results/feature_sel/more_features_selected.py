...
columns_X=['std', 'mad', 'q01', 'q05', 'q95', 'q99', 'abs_mean', 'abs_std',
'abs_mad', 'abs_q95', 'abs_q99',
'std_mean_100', 'mad_mean_10', 'q01_mean_10','q05_mean_10','q95_mean_10',
'q99_mean_10',
'std_mean_100', 'min_mean_100', 'mad_mean_100']
...
X_test = pd.DataFrame(columns=columns_X, dtype=np.float64, index=submission.index)
...
#        X_test.loc[seg_id, 'min_mean_{window}'] = data_roll_mean.min().item()
        X_test.loc[seg_id, 'mad_mean_' + str(window)] = data_roll_mean.mad().item()
...
#        X_test.loc[seg_id, 'median_mean_{window}'] = data_roll_mean.median().item()
        X_test.loc[seg_id, 'q01_mean_' + str(window)] = np.quantile(data_roll_mean, 0.01)
        X_test.loc[seg_id, 'q05_mean_' + str(window)] = np.quantile(data_roll_mean, 0.05)
...
#        X_test.loc[seg_id, 'mean_mean_'] = data_roll_mean.mean().item()
        X_test.loc[seg_id, 'std_mean_' + str(window)] = data_roll_mean.std().item()
#        X_test.loc[seg_id, 'max_mean_{window}'] = data_roll_mean.max().item()
        X_test.loc[seg_id, 'min_mean_' + str(window)] = data_roll_mean.min().item()
        X_test.loc[seg_id, 'mad_mean_' + str(window)] = data_roll_mean.mad().item()
#        X_test.loc[seg_id, 'kurt_mean_{window}'] = data_roll_mean.kurtosis().item()
#        X_test.loc[seg_id, 'skew_mean_{window}'] = data_roll_mean.skew().item()
#        X_test.loc[seg_id, 'median_mean_{window}'] = data_roll_mean.median().item()
